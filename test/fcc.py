...
def fcc():
    ...
    pos_offset = np.multiply(
        [1, 1, 1], (R_true * np.sqrt(2) / 2)
    )  ###possible values of offset that defines polymer position. will be used to check for periodic bound
    ...
    def stressxy(timestep):
        ...
        for i in np.arange(N_bonds):
            ...
            distance = apos - bpos
            force = harmonic.forces[b].force[
                1
            ]  ##y-component of force in direction toward particle a
    ...
    snapshot.particles.diameter[:] = [2 * R_true] * N_colloid + [
        2 * R_polymer
    ] * N_polymer * N_colloid
    ...
    for i in np.arange(N_colloid):
        ...
        snapshot.particles.position[i + N_colloid * 1] = [
            pos[0],
            pos[1],
            center[2],
        ]  ##x-y plane attachment
        ...
        snapshot.particles.position[i + N_colloid * 5] = [
            center[0],
            pos[1],
            pos[2],
        ]  ##y-z plane attachment
        ...
        snapshot.particles.position[i + N_colloid * 9] = [
            pos[0],
            center[1],
            pos[2],
        ]  ##x-z plane attachment
    ...
    for i in np.arange(N_colloid, N_tot):
        for j in np.arange(i + 1, N_tot):
            ...
            delta = dist(
                bead1=snapshot.particles.position[i],
                bead2=snapshot.particles.position[j],
            )
    ...
    def slj_pc(r, rmin, rmax, delta, epsilon, sigma):
        ...
        F = (
            ...
            * epsilon
            / (r - delta)
            * (12 * (sigma / (r - delta)) ** 12 - 6 * (sigma / (r - delta)) ** 6)
        )
        return (V, F)
    ...
    def slj_cc(r, rmin, rmax, delta, epsilon, sigma):
        V = (
            4
            * epsilon
            * ((sigma / (r - delta)) ** 12 - (sigma / (r - delta)) ** 6 + 1 / 4)
        )
        F = (
            4
            * epsilon
